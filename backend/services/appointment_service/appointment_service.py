...
from ...api.models import Appointment, ServiceHistory, LoyaltyTransaction, Customer, Service, Box
...
class AppointmentService:
    ...
    def create_appointment(data, user=None):
        ...
        try:
            ...
            available_box = AppointmentService._find_available_box(appointment_date, appointment_time, service)
            if not available_box:
                return {
                    'success': False,
                    'error': 'На цей час немає доступних боксів. Спробуйте інший час або дату.'
                }
            ...
            if user:
                ...
                if hasattr(user, 'customer') and user.customer:
                    final_price = user.customer.apply_discount_to_price(original_price)
                    print(f"Застосовано знижку для користувача {user.username}: {original_price} -> {final_price}")
                else:
                    ...
                    final_price = customer.apply_discount_to_price(original_price)
                    print(f"Створено customer профіль та застосовано знижку для користувача {user.username}: {original_price} -> {final_price}")
            else:
                print(f"Користувач не авторизований, знижка не застосовується: {original_price}")
    ...
    def update_appointment(appointment_id, data, user=None):
        ...
        try:
            ...
            appointment = DataAccessLayer.get_appointment_by_id(appointment_id, user)
            ...
            if user:
                ...
                if hasattr(user, 'customer') and user.customer:
                    final_price = user.customer.apply_discount_to_price(original_price)
                    print(f"Застосовано знижку при редагуванні для користувача {user.username}: {original_price} -> {final_price}")
                else:
                    # Якщо у користувача немає customer профілю, створюємо його
                    from ...api.models import Customer
                    try:
                    ...
                    # Тепер застосовуємо знижку
                    final_price = customer.apply_discount_to_price(original_price)
                    print(f"Створено customer профіль та застосовано знижку при редагуванні для користувача {user.username}: {original_price} -> {final_price}")
            else:
                print(f"Користувач не авторизований при редагуванні, знижка не застосовується: {original_price}")
            
            # Перевіряємо чи змінилася дата або час
            new_appointment_date = datetime.strptime(data['appointment_date'], '%Y-%m-%d').date()
            new_appointment_time = datetime.strptime(data['appointment_time'], '%H:%M').time()
            ...
            if date_or_time_changed:
                ...
                available_box = AppointmentService._find_available_box(new_appointment_date, new_appointment_time, service, appointment_id)
    ...
    @staticmethod
    def _find_available_box(appointment_date, appointment_time, service=None, exclude_appointment_id=None):
        """Знаходження доступного боксу для заданої дати та часу з урахуванням тривалості послуги"""
        ...
        for box in active_boxes:
            ...
            if not box.is_available_at_time(appointment_date, appointment_time):
                continue
            
            # Перевіряємо перекриття з існуючими записами
            service_duration_minutes = service.duration_minutes if service else 60
    ...
    def confirm_appointment(appointment_id):
        ...
        if appointment:
            DataAccessLayer.update_appointment(appointment, status='confirmed')
            return True
        return False
    ...
    def complete_appointment(appointment_id, completion_data):
        ...
        if appointment:
            ...
            DataAccessLayer.create_service_history(
                appointment=appointment,
                actual_duration=completion_data.get('actual_duration', appointment.service.duration),
                final_price=completion_data.get('final_price', appointment.total_price),
            ...
            if appointment.customer:
                ...
                DataAccessLayer.create_loyalty_transaction(
                    ...
                    description=f'Нарахування за послугу: {appointment.service.name}'
                )
