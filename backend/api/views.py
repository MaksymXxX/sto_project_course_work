...
from .serializers import (
    ServiceCategorySerializer, ServiceSerializer, ServiceListSerializer, CustomerSerializer, AppointmentSerializer,
    ServiceHistorySerializer, LoyaltyTransactionSerializer, STOInfoSerializer,
    GuestAppointmentSerializer, UserRegistrationSerializer, UserLoginSerializer, BoxSerializer
)
...
class ServiceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return Service.objects.filter(is_active=True).select_related('category').order_by('category__order', 'category__name', 'name')
    ...
    def featured(self, request):
        ...
        featured_services = Service.objects.filter(is_featured=True, is_active=True).select_related('category').order_by('category__order', 'category__name', 'name')
...
class STOInfoViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if queryset:
            ...
            return Response(serializer.data)
        return Response({'error': 'Інформацію про СТО не знайдено'}, status=status.HTTP_404_NOT_FOUND)
...
class GuestAppointmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Appointment.objects.filter(customer__isnull=True)
        ...
        if request.user.is_authenticated:
            try:
                ...
                if customer.is_blocked:
                    return Response({'error': 'Ваш акаунт заблокований. Ви не можете створювати записи. Зверніться до адміністратора для розблокування.'}, status=status.HTTP_403_FORBIDDEN)
...
class CustomerViewSet(viewsets.ModelViewSet):
    ...
    def profile(self, request):
        ...
        if profile_data:
            ...
            if customer.is_blocked:
                ...
                profile_data['block_message'] = 'Ваш акаунт заблокований. Зверніться до адміністратора для розблокування.'
            else:
    ...
    def update_profile(self, request):
        ...
        try:
            customer = Customer.objects.get(user=request.user)
            if customer.is_blocked:
                return Response({'error': 'Ваш акаунт заблокований. Ви не можете оновлювати профіль. Зверніться до адміністратора для розблокування.'}, status=status.HTTP_403_FORBIDDEN)
        except Customer.DoesNotExist:
            # Якщо профіль клієнта не знайдено, створюємо його
            customer = Customer.objects.create(user=request.user)
            # Після створення профілю перевіряємо чи він не заблокований
        ...
        return Response(
            {'error': 'Помилка оновлення профілю. Можливо, email вже використовується.'}, 
...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_staff:
            return Appointment.objects.all().select_related('customer__user', 'service', 'box')
    ...
    def get_object(self):
        ...
        # Якщо користувач є адміністратором, дозволяємо отримати будь-який запис
        if self.request.user.is_staff:
    ...
    def create(self, request, *args, **kwargs):
        ...
        result = AppointmentService.create_appointment(request.data, request.user)
        if result['success']:
            serializer = self.get_serializer(result['appointment'], context=self.get_serializer_context())
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response({'error': result['error']}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            customer = Customer.objects.get(user=request.user)
            if customer.is_blocked:
                return Response({'error': 'Ваш акаунт заблокований. Ви не можете редагувати записи. Зверніться до адміністратора для розблокування.'}, status=status.HTTP_403_FORBIDDEN)
        except Customer.DoesNotExist:
            # Якщо профіль клієнта не знайдено, створюємо його
            customer = Customer.objects.create(user=request.user)
            # Після створення профілю перевіряємо чи він не заблокований
        ...
        if appointment.status not in ['pending']:
            return Response({'error': 'Не можна редагувати запис у поточному статусі'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if time_difference.total_seconds() < 7200:  # 2 години = 7200 секунд
            return Response({
                'error': 'Не можна редагувати запис менше ніж за 2 години до його початку. Зверніться до адміністратора для внесення змін.'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        # Використовуємо AppointmentService для оновлення
        result = AppointmentService.update_appointment(appointment.id, request.data, request.user)
        if result['success']:
            serializer = self.get_serializer(result['appointment'], context=self.get_serializer_context())
    ...
    def confirm(self, request, pk=None):
        ...
        try:
            appointment = self.get_object()
            print(f"Підтвердження запису {appointment.id} адміністратором {request.user.username}")
            
            # Перевіряємо чи можна підтвердити запис
            if appointment.status not in ['pending']:
                return Response({'error': 'Не можна підтвердити запис у поточному статусі'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({'error': 'Помилка при підтвердженні запису'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def complete(self, request, pk=None):
        ...
        try:
            appointment = self.get_object()
            print(f"Завершення запису {appointment.id} адміністратором {request.user.username}")
            
            # Перевіряємо чи можна завершити запис
            if appointment.status not in ['confirmed', 'in_progress']:
                return Response({'error': 'Не можна завершити запис у поточному статусі'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({'error': 'Помилка при завершенні запису'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def cancel(self, request, pk=None):
        ...
        try:
            customer = Customer.objects.get(user=request.user)
            if customer.is_blocked:
                return Response({'error': 'Ваш акаунт заблокований. Ви не можете скасовувати записи. Зверніться до адміністратора для розблокування.'}, status=status.HTTP_403_FORBIDDEN)
        except Customer.DoesNotExist:
            # Якщо профіль клієнта не знайдено, створюємо його
            customer = Customer.objects.create(user=request.user)
            # Після створення профілю перевіряємо чи він не заблокований
        ...
        if appointment.status not in ['pending', 'confirmed']:
            return Response({'error': 'Не можна скасувати запис у поточному статусі'}, status=status.HTTP_400_BAD_REQUEST)
...
class ServiceHistoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ServiceHistory.objects.filter(appointment__customer__user=self.request.user)
...
class LoyaltyTransactionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return LoyaltyTransaction.objects.filter(customer__user=self.request.user)
...
class BoxViewSet(viewsets.ModelViewSet):
    ...
    def available_boxes(self, request):
        ...
        if not date_str or not time_str:
            return Response({'error': 'Потрібно вказати параметри date та time'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            appointment_date = datetime.strptime(date_str, '%Y-%m-%d').date()
            appointment_time = datetime.strptime(time_str, '%H:%M').time()
        except ValueError:
            return Response({'error': 'Неправильний формат дати або часу'}, status=status.HTTP_400_BAD_REQUEST)
        available_boxes = []
    ...
    def available_dates(self, request):
        """Отримання доступних дат на наступні 30 днів з урахуванням робочих годин боксів та тривалості послуги"""
        from datetime import timedelta
        service_id = request.query_params.get('service_id')
        exclude_appointment_id = request.query_params.get('exclude_appointment_id')
        
        if not service_id:
            return Response({'error': 'Потрібно вказати service_id'}, status=400)
        ...
        for i in range(30):
            ...
            # Перевіряємо чи є хоча б один активний бокс з вільним часом на цю дату
            ...
            for box in active_boxes:
                ...
                if working_hours and working_hours.get('start') != '00:00' and working_hours.get('end') != '00:00':
                    ...
                    while current_minutes + service_duration_minutes <= end_minutes:
                        time_slot = f"{current_minutes // 60:02d}:{current_minutes % 60:02d}"
                        ...
                        for appointment in existing_appointments:
                            ...
                            appointment_start_minutes = appointment_start.hour * 60 + appointment_start.minute
                            appointment_end_minutes = appointment_start_minutes + (appointment.service.duration_minutes or 60)
...
class AdminViewSet(viewsets.ViewSet):
    ...
    def get_serializer_context(self):
        """Додаємо мову в контекст серіалізатора"""
        try:
            # ViewSet doesn't have get_serializer_context by default, so we create our own
            ...
            import traceback
            print(f"Error in AdminViewSet.get_serializer_context: {str(e)}")
    ...
    def statistics(self, request):
        ...
        stats = {
            ...
            'blocked_customers': Customer.objects.filter(is_blocked=True).count(),
    ...
    def weekly_schedule(self, request):
        ...
        for i in range(7):
            ...
            day_name_translated = day_translations.get(day_name, {}).get(language, day_name)
            
            day_appointments = weekly_appointments.filter(appointment_date=current_date)
    ...
    @action(detail=False, methods=['get'])
    def customer_management(self, request):
    ...
    @action(detail=False, methods=['get'])
    def services_management(self, request):
        """Отримання списку всіх послуг для управління"""
        services = Service.objects.select_related('category').all()
        context = self.get_serializer_context()
        serializer = ServiceListSerializer(services, many=True, context=context)
        return Response(serializer.data)
    ...
    def get_service_for_edit(self, request, pk=None):
        ...
        try:
            ...
            return Response({'error': 'Послугу не знайдено'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def update_service(self, request, pk=None):
        ...
        try:
            ...
            context = self.get_serializer_context()
            context['admin_panel'] = True
            serializer = ServiceSerializer(service, data=request.data, partial=True, context=context)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    @action(detail=True, methods=['post'])
    def toggle_service_status(self, request, pk=None):
        ...
        try:
            ...
            service.save()
            return Response({
                'message': f'Послугу {"активовано" if service.is_active else "деактивовано"}',
    ...
    @action(detail=False, methods=['get'])
    def categories_management(self, request):
        """Отримання списку всіх категорій для управління"""
        categories = ServiceCategory.objects.all()
        context = self.get_serializer_context()
        context['admin_panel'] = True
        serializer = ServiceCategorySerializer(categories, many=True, context=context)
        return Response(serializer.data)
    ...
    def create_category(self, request):
        ...
        if 'order' not in data or data['order'] == '' or data['order'] is None:
            max_order = ServiceCategory.objects.aggregate(Max('order'))['order__max'] or 0
    ...
    def update_category(self, request, pk=None):
        ...
        try:
            ...
            context = self.get_serializer_context()
            context['admin_panel'] = True
            serializer = ServiceCategorySerializer(category, data=request.data, partial=True, context=context)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    @action(detail=False, methods=['get'])
    def boxes_management(self, request):
        """Отримання списку боксів"""
        try:
            boxes = Box.objects.all()
            serializer = BoxSerializer(boxes, many=True, context=self.get_serializer_context())
            return Response(serializer.data)
        except Exception as e:
    ...
    def create_box(self, request):
        ...
        try:
            serializer = BoxSerializer(data=request.data, context=self.get_serializer_context())
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update_box(self, request, pk=None):
        ...
        try:
            ...
            serializer = BoxSerializer(box, data=request.data, partial=True, context=self.get_serializer_context())
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        except Box.DoesNotExist:
            return Response({'error': 'Бокс не знайдено'}, status=status.HTTP_404_NOT_FOUND)

    @action(detail=True, methods=['delete'])
    def delete_box(self, request, pk=None):
        """Видалення боксу"""
        try:
            box = Box.objects.get(id=pk)
            box.delete()
    ...
    @action(detail=True, methods=['post'])
    def toggle_box_status(self, request, pk=None):
        """Зміна статусу боксу"""
        try:
            box = Box.objects.get(id=pk)
            box.is_active = not box.is_active
            box.save()
            return Response({'message': f'Бокс {"активовано" if box.is_active else "деактивовано"}'})
        except Box.DoesNotExist:
            return Response({'error': 'Бокс не знайдено'}, status=status.HTTP_404_NOT_FOUND)

    @action(detail=False, methods=['get'])
    def home_page_management(self, request):
        """Отримання інформації про головну сторінку для адміністратора"""
        try:
            sto_info = ServiceCatalog.get_sto_info()
            if sto_info:
                serializer = STOInfoSerializer(sto_info, context=self.get_serializer_context())
                return Response(serializer.data)
            else:
                return Response({'error': 'Інформація про СТО не знайдена'}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            print(f"Error in home_page_management: {str(e)}")
            return Response({'error': f'Помилка отримання даних: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def update_home_page(self, request):
        ...
        try:
            ...
            print(f"Error updating home page: {str(e)}")
            return Response({'error': f'Помилка оновлення: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    
    @action(detail=False, methods=['get'])
    def featured_services(self, request):
        """Отримання основних послуг для головної сторінки"""
        featured_services = Service.objects.filter(is_featured=True, is_active=True)
        serializer = ServiceSerializer(featured_services, many=True, context=self.get_serializer_context())
        return Response(serializer.data)
    
    @action(detail=True, methods=['post'])
    def toggle_featured_service(self, request, pk=None):
        ...
        try:
            ...
            service.save()
            return Response({
                'message': f'Послугу {"додано до основних" if service.is_featured else "видалено з основних"}',
    ...
    def appointments(self, request):
        ...
        serializer = AppointmentSerializer(queryset, many=True, context=self.get_serializer_context())
    ...
    def appointment_details(self, request, pk=None):
        ...
        try:
            ...
            from .serializers import AppointmentSerializer
            serializer = AppointmentSerializer(appointment, context=self.get_serializer_context())
            ...
            import traceback
            print(f"Error in appointment_details: {str(e)}")
            print(f"Traceback: {traceback.format_exc()}")
        ...
        try:
            ...
            if 'email' in request.data and request.data['email'] != user.email:
                if User.objects.filter(email=request.data['email']).exclude(id=user.id).exists():
                    return Response({'error': 'Email вже використовується'}, status=status.HTTP_400_BAD_REQUEST)
                ...
                try:
                    ...
                    for error in e.error_list:
                        if 'too common' in str(error).lower():
                            ...
                            error_messages.append('Пароль занадто схожий на особисті дані')
                        elif 'numeric' in str(error).lower():
                            error_messages.append('Пароль не може складатися тільки з цифр')
                        else:
                            error_messages.append(str(error))
                    
                    return Response({'error': 'Помилка валідації пароля: ' + '; '.join(error_messages)}, status=status.HTTP_400_BAD_REQUEST)
