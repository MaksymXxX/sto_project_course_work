...
class BoxSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_active', 'working_hours', 'created_at', 'updated_at'
        ]
...
class ServiceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Service
        fields = [
            'id', 'category', 'category_id', 'name', 'name_en',
            'description', 'description_en', 'price', 'duration_minutes',
    ...
    def to_representation(self, instance):
        ...
        if instance.category:
            admin_panel = self.context.get('admin_panel', False)
            data['category'] = ServiceCategorySerializer(
                instance.category,
                context={'language': language, 'admin_panel': admin_panel}
            ).data
...
class ServiceListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'duration_minutes', 'is_active', 'is_featured', 'created_at'
        ]
...
class CustomerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Customer
        fields = [
            'id', 'user', 'address', 'avatar', 'loyalty_points',
...
class AppointmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'service', 'service_id', 'box', 'box_id', 'appointment_date',
            'appointment_time', 'status', 'notes', 'total_price',
    ...
    def to_representation(self, instance):
        ...
        try:
            ...
            import traceback
            error_msg = f"Error in AppointmentSerializer.to_representation: {str(e)}"
            print(error_msg)
...
class ServiceHistorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'appointment', 'completed_at', 'mechanic_notes',
...
class LoyaltyTransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'customer', 'transaction_type', 'points',
...
class STOInfoSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        data['what_you_can_title'] = instance.get_what_you_can_title(
            language)
        data['what_you_can_items'] = instance.get_what_you_can_items_list(
            language)

        # Адреса, телефон, email за мовою (з фолбеком)
        if language == 'en' and getattr(instance, 'address_en', None):
            data['address'] = instance.address_en
        else:
            data['address'] = instance.address

        if language == 'en' and getattr(instance, 'phone_en', None):
            data['phone'] = instance.phone_en
        else:
            data['phone'] = instance.phone

        if language == 'en' and getattr(instance, 'email_en', None):
            data['email'] = instance.email_en
        else:
            data['email'] = instance.email

        # Робочі години: повертаємо тиждень/вихідні окремо якщо можливо
        if language == 'en' and getattr(instance, 'working_hours_en', None):
            working_hours_raw = instance.working_hours_en
        else:
            working_hours_raw = instance.working_hours
        if isinstance(working_hours_raw, str) and ',' in working_hours_raw:
            ...
            # Якщо немає другої частини, не додаємо weekend
            # або ставимо порожнє значення
        ...
        if isinstance(data['what_you_can_items'], str):
            try:
                import json
                data['what_you_can_items'] = json.loads(
...
class GuestAppointmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'service', 'service_id', 'box', 'box_id', 'appointment_date',
            'appointment_time', 'notes', 'total_price', 'created_at'
        ]
